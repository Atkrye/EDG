...
static void evaluatePredicate6(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) b6 = true;
   else b6 = false;
}
...
static void evaluatePredicate7(Morphism *morphism)
{
   ...
   if(getOutdegree(host, n0) < var_4) b7 = true;
   else b7 = false;
}
...
bool matchMain_addEdgeToArityBlue(Morphism *morphism)
{
   if(3 > host->number_of_nodes || 0 > host->number_of_edges) return false;
...
static bool match_n0(Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         if(evaluateCondition()) next_match_result = match_n2(morphism);
...
void applyMain_addEdgeToArityBlue(Morphism *morphism, bool record_changes)
{
...
bool fillpotMain_addEdgeToArityBlue(MorphismPot *pot, Morphism *morphism)
{
   if(3 > host->number_of_nodes || 0 > host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         evaluatePredicate7(morphism);
         bool next_match_result = false;
         if(evaluateCondition()) next_match_result = fillpot_n2(pot, morphism);
         if(next_match_result) return true;
         else
         {
            /* Reset the boolean variables in the predicates of this node. */
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "Main_addEdgeToArityBlue", host);
