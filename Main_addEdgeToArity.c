...
static void evaluatePredicate2(Morphism *morphism)
{
   ...
   if(getOutdegree(host, n0) < var_4) generateIndividual_b2 = true;
...
bool matchMain_addEdgeToArity(Morphism *morphism)
{
...
void applyMain_addEdgeToArity(Morphism *morphism, bool record_changes)
{
...
bool fillpotMain_addEdgeToArity(MorphismPot *pot, Morphism *morphism)
{
   if(3 > host->number_of_nodes || 0 > host->number_of_edges) return false;
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "Main_addEdgeToArity", host);
