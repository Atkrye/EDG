...
bool matchMain_initNodeCounter(Morphism *morphism)
{
...
void applyMain_initNodeCounter(Morphism *morphism, bool record_changes)
{
...
bool fillpotMain_initNodeCounter(MorphismPot *pot, Morphism *morphism)
{
   if(1 > host->number_of_nodes || 0 > host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "Main_initNodeCounter", host);
