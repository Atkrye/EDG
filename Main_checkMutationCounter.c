...
bool mutateIndividual_b0 = true;

static bool evaluateCondition(void)
{
   return (mutateIndividual_b0);
}
...
static void evaluatePredicate0(Morphism *morphism)
{
   ...
   if(var_1 < var_0) mutateIndividual_b0 = true;
   else mutateIndividual_b0 = false;
}
...
static bool match_n1(Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(host); nodes != NULL; nodes = nodes->next)
   {
      ...
      do
      {
         ...
         /* Matching rule atom 1. */
         if(item->atom.type != 's') break;
         else if(strcmp(item->atom.str, "Mutated") != 0) break;
         item = item->next;
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "Main_checkMutationCounter", host);
