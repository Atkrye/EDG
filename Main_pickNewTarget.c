...
bool mutateIndividual_b1 = true;

static bool evaluateCondition(void)
{
   return (mutateIndividual_b1);
}
...
static void evaluatePredicate1(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) mutateIndividual_b1 = true;
   else mutateIndividual_b1 = false;
}
...
void applyMain_pickNewTarget(Morphism *morphism, bool record_changes)
{
   ...
   changeNodeMark(host, host_node_index, 1);
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "Main_pickNewTarget", host);
