...
{
   if(2 > host->number_of_nodes || 1 > host->number_of_edges) return false;
...
void applyMain_pickEdge(Morphism *morphism, bool record_changes)
{
   ...
   changeEdgeMark(host, host_edge_index, 2);
   ...
   changeNodeMark(host, host_node_index, 2);

   host_node_index = lookupNode(morphism, 1);
   Node *node1 = getNode(host, host_node_index);
   if(!node1->root)
   {
      if(record_changes) pushChangedRootNode(host_node_index);
      changeRoot(host, host_node_index);
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "Main_pickEdge", host);
