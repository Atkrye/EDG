...
bool neutral_b4 = true;
bool neutral_b5 = true;

static bool evaluateCondition(void)
{
   return (neutral_b4 && neutral_b5);
}
...
static void evaluatePredicate4(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) neutral_b4 = true;
   else neutral_b4 = false;
}
...
static void evaluatePredicate5(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) neutral_b5 = true;
   else neutral_b5 = false;
}
...
{
   if(6 > host->number_of_nodes || 3 > host->number_of_edges) return false;
...
static bool match_n1(Morphism *morphism, Edge *host_edge)
{
   ...
   if(host_node->label.mark != 3) return false;
      if(host_node->indegree < 1 || host_node->outdegree < 2 ||
         ((host_node->outdegree + host_node->indegree - 2 - 1 - 0) < 0)) return false;;
   ...
   do
   {
      ...
      /* Matching rule atom 1. */
      if(item->atom.type != 's') break;
      else if(strcmp(item->atom.str, "OR") != 0) break;
      match = true;
   } while(false);
...
void applyMain_demorgansF2(Morphism *morphism, bool record_changes)
{
   ...
   host_node_index = lookupNode(morphism, 4);
   HostLabel label_n4 = getNodeLabel(host, host_node_index);
   int list_var_length1 = 0;
   int list_length1 = list_var_length1 + 1;
   HostAtom array1[list_length1];
   int index1 = 0;
   ...
   if(equalHostLabels(label_n4, label)) removeHostList(label.list);
   else
   {
      if(record_changes) pushRelabelledNode(host_node_index, label_n4);
      relabelNode(host, host_node_index, label);
   }
   ...
   int edge_array_size1 = host->edges.size;
   source = lookupNode(morphism, 0);
   ...
   /* If the edge array size has not increased after the edge addition, then
      the edge was added to a hole in the array. */
   if(record_changes)
      pushAddedEdge(host_edge_index, edge_array_size1 == host->edges.size);
   int edge_array_size2 = host->edges.size;
   source = lookupNode(morphism, 4);
   target = lookupNode(morphism, 2);
   int list_var_length5 = 0;
   int list_length5 = list_var_length5 + 1;
   ...
   if(list_length5 > 0)
   {
      ...
      label = makeHostLabel(1, list_length5, list5);
   }
   else label = makeEmptyLabel(1);
   ...
   /* If the edge array size has not increased after the edge addition, then
      the edge was added to a hole in the array. */
   if(record_changes)
      pushAddedEdge(host_edge_index, edge_array_size2 == host->edges.size);
   int edge_array_size3 = host->edges.size;
   source = lookupNode(morphism, 5);
   target = lookupNode(morphism, 3);
   int list_var_length6 = 0;
   int list_length6 = list_var_length6 + 1;
   ...
   if(list_length6 > 0)
   {
      ...
      label = makeHostLabel(1, list_length6, list6);
   }
   else label = makeEmptyLabel(1);
...
static bool fillpot_n2(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      addNodeMap(morphism, 2, host_node->index, new_assignments);
      host_node->matched = true;
      if(fillpot_n4(pot, morphism)) return true;
      else
      {
      removeNodeMap(morphism, 2);
      host_node->matched = false;
...
static bool fillpot_n5(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "Main_demorgansF2", host);
