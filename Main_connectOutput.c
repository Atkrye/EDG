...
static void evaluatePredicate4(Morphism *morphism)
{
   ...
   if(getOutdegree(host, n0) < 1) generateIndividual_b4 = true;
...
bool matchMain_connectOutput(Morphism *morphism)
{
...
void applyMain_connectOutput(Morphism *morphism, bool record_changes)
{
   int host_edge_index;
   int source, target;
...
bool fillpotMain_connectOutput(MorphismPot *pot, Morphism *morphism)
{
   if(2 > host->number_of_nodes || 0 > host->number_of_edges) return false;
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "Main_connectOutput", host);
