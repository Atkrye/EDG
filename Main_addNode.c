...
bool matchMain_addNode(Morphism *morphism)
{
...
void applyMain_addNode(Morphism *morphism, bool record_changes)
{
   ...
   int node_array_size2 = host->nodes.size;
   ...
   if(record_changes)
      pushAddedNode(host_node_index, node_array_size2 == host->nodes.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, host);
}
...
bool fillpotMain_addNode(MorphismPot *pot, Morphism *morphism)
{
   if(2 > host->number_of_nodes || 0 > host->number_of_edges) return false;
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "Main_addNode", host);
