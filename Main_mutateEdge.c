...
bool mutateIndividual_b2 = true;

static bool evaluateCondition(void)
{
   return (mutateIndividual_b2);
}
...
static void evaluatePredicate2(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) mutateIndividual_b2 = true;
   else mutateIndividual_b2 = false;
}
...
{
   if(3 > host->number_of_nodes || 1 > host->number_of_edges) return false;
...
void applyMain_mutateEdge(Morphism *morphism, bool record_changes)
{
   ...
   changeNodeMark(host, host_node_index, 0);
...
static bool fillpot_n2(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "Main_mutateEdge", host);
