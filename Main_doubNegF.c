...
bool neutral_b6 = true;
bool neutral_b7 = true;

static bool evaluateCondition(void)
{
   return (neutral_b6 && neutral_b7);
}

static void evaluatePredicate6(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) neutral_b6 = true;
   else neutral_b6 = false;
}

static void evaluatePredicate7(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) neutral_b7 = true;
   else neutral_b7 = false;
}
...
bool matchMain_doubNegF(Morphism *morphism)
{
...
void applyMain_doubNegF(Morphism *morphism, bool record_changes)
{
   ...
   host_node_index = lookupNode(morphism, 3);
   HostLabel label_n3 = getNodeLabel(host, host_node_index);
   int list_var_length1 = 0;
   int list_length1 = list_var_length1 + 1;
   HostAtom array1[list_length1];
   int index1 = 0;
   ...
   if(equalHostLabels(label_n3, label)) removeHostList(label.list);
   else
   {
      if(record_changes) pushRelabelledNode(host_node_index, label_n3);
      relabelNode(host, host_node_index, label);
   }
   int source, target;
   ...
   list_var_length2 += getAssignmentLength(var_4);
   ...
   {
      HostListItem *item2 = var_4.list->first;
      while(item2 != NULL)
      {
   ...
   else if(var_4.type == 'i')
   {
      ...
      array2[index2++].num = var_4.num;
   }
   else if(var_4.type == 's')
   {
      array2[index2].type = 's';
      array2[index2++].str = var_4.str;
   }
...
bool fillpotMain_doubNegF(MorphismPot *pot, Morphism *morphism)
{
   if(4 > host->number_of_nodes || 1 > host->number_of_edges) return false;
...
static bool fillpot_n3(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "Main_doubNegF", host);
